...
const getVariableAssignmentsMemoized = memoizeOne(
  (
    ...
    return Object.entries(valuesState.values).map(([variableID, values]) => {
      const variableName = variablesState[variableID].variable.name

      return getVarAssignment(variableName, values)
    })
  }
)
